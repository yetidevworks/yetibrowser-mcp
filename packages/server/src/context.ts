...
export class ExtensionContext {
  ...
  async diffLatestSnapshots(): Promise<ToolResponse> {
    ...
    const formatEntry = (entry: DomSnapshotEntry) =>
...
function diffSnapshots(previous: DomSnapshot, current: DomSnapshot): SnapshotDiff {
  const prevMap = new Map<string, DomSnapshotEntry>();
  const currentMap = new Map<string, DomSnapshotEntry>();
  ...
  const added: DomSnapshotEntry[] = [];
  const removed: DomSnapshotEntry[] = [];
  const changed: Array<{ selector: string; before: DomSnapshotEntry; after: DomSnapshotEntry }> = [];
