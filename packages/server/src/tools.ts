...
export function createTools(): Tool[] {
  ...
  const waitForTool: Tool = {
    schema: {
      ...
      description:
        "Wait until a selector appears (optionally visible) before continuing. Returns a fresh snapshot after the element is detected.",
  ...
  const fillFormTool: Tool = {
    schema: {
      ...
      description:
        "Fill multiple form fields in a single call. Supports inputs, textareas, selects, checkboxes, and radios.",
  ...
  const evaluateTool: Tool = {
    schema: {
      ...
      description:
        "Run custom JavaScript inside the page context and return the JSON-serializable result.",
  ...
  const handleDialogTool: Tool = {
    schema: {
      ...
      description:
        "Accept or dismiss the currently open alert/confirm/prompt dialog in the active tab.",
    ...
    handle: async (context, params) => {
      ...
      const summary = `Dialog ${action === "accept" ? "accepted" : "dismissed"}${
        promptText ? ` with prompt text "${promptText}"` : ""
      }.`;
      return {
        content: [
          {
